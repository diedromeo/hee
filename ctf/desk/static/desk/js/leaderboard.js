...
function updateScore(contestants) {
    ...
    if (contestants.length > 0) {
        contestants.forEach((contestant, index) => {
            ...
            innerCode += `<tr id="${contestant.contestant}"><td class="number">${index + 1}</td><td class="name">${contestant.contestant__team_name}<p class='submission-time'>Last Flag Submit at ${new Date(contestant.last * 1000).toLocaleTimeString([], {
